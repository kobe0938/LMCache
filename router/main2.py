    ...
    while True:
        ...
        try:
            with httpx.Client() as client:  # Use synchronous client
                ...
                machine_queues.task_done()
                return response.json()  # Return the actual response from the GPU server
...
async def route_request(request: Request):
    try:
        ...
        print(f"queued, Current queue size: {machine_queues.qsize()}")
        return {"status": "queued", "queue_size": machine_queues.qsize()}
